...
const ReloadCustomers = ({ count, queryString }) => {
  ...
  useEffect(() => {
    ...
    const checkNewData = async () => {
      try {
        const response = await fetch(`${API_Allergyjom}/api/v1/order/count?${queryString}`);
      ...
      <td className={twMerge(commonCellClasses)}>
        <div className="flex flex-col gap-2">
          <ActionButton className="bg-green-300 hover:bg-green-500" onClick={async () => {
            try {
              const response = await fetch(`${API_Allergyjom}/api/v1/order/${order?.id}/status/shipped`, {
                method: "PATCH",
          ...
          <ActionButton className="bg-green-600 hover:bg-green-800" onClick={async () => {
            try {
              const response = await fetch(`${API_Allergyjom}/api/v1/order/${order?.id}/status/delivered`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                // body: JSON.stringify({ status: "completed" })
              });
          ...
          <ActionButton className="bg-orange-500 hover:bg-orange-700" onClick={async () => {
            try {
              const response = await fetch(`${API_Allergyjom}/api/v1/order/${order?.id}/status/canceled`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
          ...
          <ActionButton type="button" className="bg-red-700 hover:bg-red-900" onClick={async () => {
            try {
              const response = await fetch(`${API_Allergyjom}/api/v1/order/${order?.id}`, {
                method: "DELETE",
              })
              if (response.ok) {
                setCustomer(null);
...
const AllergyjomCustomers = () => {
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_Allergyjom}/api/v1/order?${queryString}`);
